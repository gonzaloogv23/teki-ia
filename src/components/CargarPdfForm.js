...
const CargarPdfForm = ({ setPdfFile }) => {
  ...
  const [pdfFile, setPdfFileState] = useState(null);
  ...
  const handlePdfChange = (event) => {
    ...
    setPdfFileState(event.target.files[0]);
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      const storageRef = ref(storage, `conocimientos/${pdfFile.name}`);
      await uploadBytes(storageRef, pdfFile);
      const downloadURL = await getDownloadURL(storageRef);

      // Guarda el enlace en Firestore
      ...
      alert("PDF subido correctamente y guardado en la base de datos");
      setPdfFile(null);
  ...
  return (
    <div className="modal">
      ...
      <div className="modal-body">
        ...
        <button className="upload-area" onClick={handleFileClick}>
        ...
        <input
          ...
          style={{ display: 'none' }}
        />
        {isUploading && <p>Subiendo el archivo...</p>}
      </div>
      <div className="modal-footer">
        <button className="btn-secondary" onClick={() => setPdfFile(null)}>
          Cancelar
        </button>
        <button className="btn-primary" onClick={handleUpload} disabled={!pdfFile || isUploading}>
          Enviar PDF
        </button>
      </div>
    </div>
